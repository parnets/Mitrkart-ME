...
import com.Mitrakarte_ME.meappmintrakarte.Activity.KeyAccountProductViewActivity;
...
public class KeyAccountFetchProduct  extends AsyncTask<Context, Void, Integer> {

    KeyAccountProductViewActivity mContext;
    Integer result = 200;
    ...
    protected Integer doInBackground(Context... params) {
        ...
        try {
            ...
            HttpPost postRequest = new HttpPost(url_base+"fetchproductKeyAccount");

            List<NameValuePair> postRequestParams = new ArrayList<>();
            mContext = (KeyAccountProductViewActivity) params[0];

            System.out.println("jyo Category sending paramter"+"mobile"+ht_inst.getUser().getkMobile()+"token"+ht_inst.getUser().getKeyAccountToken());
            ...
            if((line = reader.readLine())!=null){
                System.out.println("jyo get fetch my sub_category_product response "+line);
            }
    ...
    private void parseResult(String response) {
        try {
            ...
            for (int i = 0; i < post1.length(); i++) {
                ...
                ht_inst.getProductList2().add(catItem);

            }

            System.out.println("jyo number of sub_category_product fetched is "+ht_inst.getProductList2().size());

        }
        catch (Exception e) {
            System.out.println("jyo failed  parse fetch sub_category_product");
