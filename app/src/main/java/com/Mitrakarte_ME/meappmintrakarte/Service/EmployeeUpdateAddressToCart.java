...
public class EmployeeUpdateAddressToCart extends AsyncTask<Context, Void, Integer>  {
    ...
    protected Integer doInBackground(Context... params) {
        ...
        try {
            ...
            HttpPost postRequest = new HttpPost(url_base+"updateEmployeeAddresscart");

            List<NameValuePair> postRequestParams = new ArrayList<>();
            mContext = (EmployeeAddAddressActivity) params[0];

            System.out.println("updateEmployeeAddresscart "+"mobile   "+ht_inst.getUser().getEmpMobile()+"employee_token "+ht_inst.getUser().getEmployeeToken());
...
//            System.out.println("AddAddressActivity"+"name       "+ht_inst.getAddress().getAdr_name_k());
            ...
            postRequestParams.add(new BasicNameValuePair("name", String.valueOf(ht_inst.getAddress().getAdr_name_emp())));
            postRequestParams.add(new BasicNameValuePair("locality", String.valueOf(ht_inst.getAddress().getAdr_locality_emp())));
            postRequestParams.add(new BasicNameValuePair("land_mark", String.valueOf(ht_inst.getAddress().getAdr_land_mark_emp())));
            postRequestParams.add(new BasicNameValuePair("country", String.valueOf(ht_inst.getAddress().getAdr_country_emp())));
            postRequestParams.add(new BasicNameValuePair("state", String.valueOf(ht_inst.getAddress().getAdr_state_emp())));
            postRequestParams.add(new BasicNameValuePair("city", String.valueOf(ht_inst.getAddress().getAdr_city_emp())));
            postRequestParams.add(new BasicNameValuePair("pin", String.valueOf(ht_inst.getAddress().getAdr_pin_emp())));
