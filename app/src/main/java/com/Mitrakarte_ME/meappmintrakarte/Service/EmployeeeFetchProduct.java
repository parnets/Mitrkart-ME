...
import com.Mitrakarte_ME.meappmintrakarte.Activity.EmployeeProductViewActivity;
...
public class EmployeeeFetchProduct extends AsyncTask<Context, Void, Integer> {

    EmployeeProductViewActivity mContext;
    Integer result = 200;
    ...
    protected Integer doInBackground(Context... params) {
        ...
        try {
            ...
            HttpPost postRequest = new HttpPost(url_base+"fetchproductEmployee");

            List<NameValuePair> postRequestParams = new ArrayList<>();
            mContext = (EmployeeProductViewActivity) params[0];

            System.out.println("jyo Category sending paramter"+"mobile"+ht_inst.getUser().getEmpMobile()+"corporate_token"+ht_inst.getUser().getEmployeeToken());
            ...
            if((line = reader.readLine())!=null){
                System.out.println("jyo get fetch my subcateg_product response "+line);
            }
    ...
    private void parseResult(String response) {
        try {
            ...
            System.out.println("jyo number of subcateg_product fetched is "+ht_inst.getAllproductListCorporate().size());

        }
        catch (Exception e) {
            System.out.println("jyo failed  parse fetch subcateg_product");
