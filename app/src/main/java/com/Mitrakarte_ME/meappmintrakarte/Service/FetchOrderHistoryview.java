...
import com.Mitrakarte_ME.meappmintrakarte.Activity.KeyAccount_MyOrderActivity;
...
public class FetchOrderHistoryview extends AsyncTask<Context, Void, Integer> {
    ...
    KeyAccount_MyOrderActivity mContext;
    Integer result = 200;
    ...
    protected Integer doInBackground(Context... params) {
        ...
        try {
            ...
            HttpPost postRequest = new HttpPost(url_base+"orderHistoryViewKeyAccount");

            List<NameValuePair> postRequestParams = new ArrayList<>();
            mContext = (KeyAccount_MyOrderActivity) params[0];


            System.out.println("MyOrderActivity "+"mobile"+ht_inst.getUser().getkMobile());
            System.out.println("MyOrderActivity "+"keyaccount_token"+ht_inst.getUser().getUserToken());
            System.out.println("MyOrderActivity "+"keyaccount_id"+ht_inst.getUser().getKeyAccountId());
    ...
    private void parseResult(String response) {
        try {
            ...
            for (int i = 0; i < post1.length(); i++) {
                ...
                catItem.setGrand_price(item.getString("grand_total"));
