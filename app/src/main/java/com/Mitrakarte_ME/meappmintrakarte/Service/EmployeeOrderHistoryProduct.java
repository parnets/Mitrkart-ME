...
public class EmployeeOrderHistoryProduct extends AsyncTask<Context, Void, Integer> {

    EmployeeOrderProductViewActivity mContext;
    Integer result = 200;
    ...
    protected Integer doInBackground(Context... params) {
        ...
        try {
            ...
            HttpPost postRequest = new HttpPost(url_base+"orderHistoryProViewEmployee");

            List<NameValuePair> postRequestParams = new ArrayList<>();
            mContext = (EmployeeOrderProductViewActivity) params[0];
            ...
            System.out.println("OrderProductViewActivity    "+"employee_order_id  "+ht_inst.getTables().getOrder_id_emp());
            System.out.println("OrderProductViewActivity    "+"mobile "+ht_inst.getUser().getEmpMobile());
            System.out.println("OrderProductViewActivity     "+"employee_token "+ht_inst.getUser().getEmployeeToken());
            ...
            postRequestParams.add(new BasicNameValuePair("employee_order_id", String.valueOf(ht_inst.getTables().getOrder_id_emp())));
    ...
    private void parseResult(String response) {
        try {
            ...
            ht_inst.getEmployeeOrderHistoryProductlist().clear();
            ...
            for (int i = 0; i < post1.length(); i++) {
                ...
                catItem.setDop_id_emp(item.getString("dop_id"));
                ...
                catItem.setOrder_pro_img_emp(item.getString("pro_img"));
                catItem.setOrder_pro_name_emp(item.getString("pro_name"));
                catItem.setOrder_pro_price_emp(item.getString("pro_price"));
                catItem.setOrder_pro_offer_emp(item.getString("pro_offer"));
                catItem.setSt_qnt_emp(item.getString("pro_qnty"));
                catItem.setOrder_pro_gst_emp(item.getString("pro_gst"));
                catItem.setOrder_status_emp(item.getString("status"));
                catItem.setUpdate_on_place_order_emp(item.getString("updated_on"));
                catItem.setCreated_on_place_order_emp(item.getString("created_on"));


                ht_inst.getEmployeeOrderHistoryProductlist().add(catItem);

            }

            System.out.println("jyo number of category fetched is "+ht_inst.getEmployeeOrderHistoryProductlist().size());
