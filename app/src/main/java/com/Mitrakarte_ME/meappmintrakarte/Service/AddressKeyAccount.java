...
public class AddressKeyAccount extends AsyncTask<Context, Void, Integer> {
    ...
    protected Integer doInBackground(Context... params) {
        ...
        try {
            ...
            HttpPost postRequest = new HttpPost(url_base+"userAddressKeyAccount");

            List<NameValuePair> postRequestParams = new ArrayList<>();
            mContext = (AddAddressActivity) params[0];

            System.out.println("AddAddressActivity  " +"mobile"+ht_inst.getUser().getkMobile());
            System.out.println("AddAddressActivity  "+"keyaccount_token"+ht_inst.getUser().getKeyAccountToken());
            System.out.println("AddAddressActivity  "+"key_account_id"+ht_inst.getUser().getKeyAccountId());
    ...
    private void parseResult(String response) {
        try {
            ...
            for (int i = 0; i < post1.length(); i++) {
                ...
                udata.setAdr_ka_id(item.getInt("ka_id"));
                // catItem.setu(item.getString("key_account_id"));
                udata.setAdr_name_k(item.getString("name"));
                udata.setAdr_keyaccount_id_k(item.getInt("keyaccount_id"));
                udata.setAdr_address_k(item.getString("address"));
                udata.setAdr_locality_k(item.getString("locality"));
                udata.setAdr_land_mark_k(item.getString("land_mark"));
                udata.setAdr_country_k(item.getString("country"));
                udata.setAdr_state_k(item.getString("state"));
                udata.setAdr_city_k(item.getString("city"));
                udata.setAdr_pin_k(item.getString("pin"));

                udata.setAdr_mobile_k(item.getString("mobile"));
                udata.setAdr_email_k(item.getString("email"));
                udata.setAdr_updated_on_k(item.getString("updated_on"));
                udata.setAdr_created_on_k(item.getString("created_on"));
