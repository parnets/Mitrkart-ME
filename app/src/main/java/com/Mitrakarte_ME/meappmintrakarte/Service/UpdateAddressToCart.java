...
public class UpdateAddressToCart extends AsyncTask<Context, Void, Integer> {
    ...
    protected Integer doInBackground(Context... params) {
        ...
        try {
            ...
            HttpPost postRequest = new HttpPost(url_base+"updateuserAddresscart");

            List<NameValuePair> postRequestParams = new ArrayList<>();
            mContext = (AddAddressActivity) params[0];

            System.out.println("AddAddressActivity"+"mobile   "+ht_inst.getUser().getkMobile()+"keyaccount_token"+ht_inst.getUser().getKeyAccountToken());
...
//            System.out.println("AddAddressActivity"+"name       "+ht_inst.getAddress().getAdr_name_k());
            ...
            postRequestParams.add(new BasicNameValuePair("name", String.valueOf(ht_inst.getAddress().getAdr_name_k())));
            postRequestParams.add(new BasicNameValuePair("locality", String.valueOf(ht_inst.getAddress().getAdr_locality_k())));
            postRequestParams.add(new BasicNameValuePair("land_mark", String.valueOf(ht_inst.getAddress().getAdr_land_mark_k())));
            postRequestParams.add(new BasicNameValuePair("country", String.valueOf(ht_inst.getAddress().getAdr_country_k())));
            postRequestParams.add(new BasicNameValuePair("state", String.valueOf(ht_inst.getAddress().getAdr_state_k())));
            postRequestParams.add(new BasicNameValuePair("city", String.valueOf(ht_inst.getAddress().getAdr_city_k())));
            postRequestParams.add(new BasicNameValuePair("pin", String.valueOf(ht_inst.getAddress().getAdr_pin_k())));
