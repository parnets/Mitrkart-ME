...
public class CorporateOrderHistoryProduct extends AsyncTask<Context, Void, Integer> {

    CorporateOrderProductViewActivity mContext;
    Integer result = 200;
    ...
    protected Integer doInBackground(Context... params) {
        ...
        try {
            ...
            HttpPost postRequest = new HttpPost(url_base+"OrderHistoryProViewCorporate");

            List<NameValuePair> postRequestParams = new ArrayList<>();
            mContext = (CorporateOrderProductViewActivity) params[0];


            System.out.println("OrderProductViewActivity  "+"mobile "+ht_inst.getUser().getCrmobile());
            System.out.println("OrderProductViewActivity  "+"corporate_token "+ht_inst.getUser().getCorporateToken());
            System.out.println("OrderProductViewActivity      "+"corporate_order_id  "+ht_inst.getTables().getOrder_id_cor());
            ...
            postRequestParams.add(new BasicNameValuePair("corporate_order_id", String.valueOf(ht_inst.getTables().getOrder_id_cor())));
    ...
    private void parseResult(String response) {
        try {
            ...
            ht_inst.getCorporateOrderHistoryProductlist().clear();
            ...
            for (int i = 0; i < post1.length(); i++) {
                ...
                catItem.setDop_id_cor(item.getString("cop_id"));
                ...
                catItem.setOrder_pro_img_cor(item.getString("pro_img"));
                catItem.setOrder_pro_name_cor(item.getString("pro_name"));
                catItem.setOrder_pro_price_cor(item.getString("pro_price"));
                catItem.setOrder_pro_offer_cor(item.getString("pro_offer"));
                catItem.setSt_qnt_cor(item.getString("pro_qnty"));
                catItem.setOrder_pro_gst_cor(item.getString("pro_gst"));
                catItem.setOrder_status_cor(item.getString("status"));
                catItem.setUpdate_on_place_order_cor(item.getString("updated_on"));
                catItem.setCreated_on_place_order_cor(item.getString("created_on"));


                ht_inst.getCorporateOrderHistoryProductlist().add(catItem);

            }

            System.out.println("jyo number of category fetched is "+ht_inst.getCorporateOrderHistoryProductlist().size());
