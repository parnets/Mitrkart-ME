...
import com.Mitrakarte_ME.meappmintrakarte.Activity.KeyAccount_Product_Search_Activity;
import com.Mitrakarte_ME.meappmintrakarte.Activity.KeyAccount_Product_Search_Adapter;
...
public class KeyAccount_FetchProductSearch extends AsyncTask<Context, Void, Integer> {

    KeyAccount_Product_Search_Activity mContext;
    Integer result = 200;
    ...
    protected Integer doInBackground(Context... params) {
        ...
        try {
            ...
            HttpPost postRequest = new HttpPost(url_base + "searchproduct_keyaccount");
            ...
            postRequestParams.add(new BasicNameValuePair("token", String.valueOf(ht_inst.getUser().getKeyAccountToken())));
            postRequestParams.add(new BasicNameValuePair("user_id", String.valueOf(ht_inst.getUser().getKeyAccountId())));
            postRequestParams.add(new BasicNameValuePair("search", String.valueOf(ht_inst.getUser().getProduct_Search_key())));

            System.out.println(" Product_search sending paramter " + "mobile " + ht_inst.getUser().getkMobile());
            System.out.println(" Product_search sending paramter " + "token " + ht_inst.getUser().getKeyAccountToken());
            System.out.println(" Product_search sending paramter " + "user_id " + ht_inst.getUser().getKeyAccountId());
            System.out.println(" Product_search sending paramter " + "search " + ht_inst.getUser().getProduct_Search_key());
    ...
    private void parseResult(String response) {
        try {
            ...
            System.out.println("jyo number of category_product fetched is "+ht_inst.getMobileProductlist().size());

        }
        catch (Exception e) {
            System.out.println("jyo failed  parse fetch category_product");
        }
    }
